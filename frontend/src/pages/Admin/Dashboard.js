...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const response = await authorizedFetch(`${process.env.REACT_APP_API_BASE_URL}/api/admin/dashboard-stats`);
  ...
  useEffect(() => {
    ...
    const fetchActivities = async () => {
      try {
        const response = await authorizedFetch(`${process.env.REACT_APP_API_BASE_URL}/api/admin/live-activities?limit=5&days=2`);
  ...
  useEffect(() => {
    ...
    const fetchSmartBoard = async () => {
      try {
        const response = await authorizedFetch(`${process.env.REACT_APP_API_BASE_URL}/api/admin/smart-board?limit=20`);
